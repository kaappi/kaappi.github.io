...
| `kaappi test [PATHS...]` | Run SRFI-64 test suites (files or directories), one subprocess per file, and print an aggregate summary with a reproducible seed. `--json`, `--seed N`, `--lib-path DIR`, `-j`/`--jobs N` (run files concurrently; default one job per CPU, `--jobs 1` for strictly sequential); `--changed` / `--list-affected [--since REV]` select only suites whose import closure actually changed. |
