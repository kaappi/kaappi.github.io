...
| `eco` | `sweep_eco.py` | ecosystem pages, service-backed: live redis + PostgreSQL, workspace dylibs, per-block connection/schema preludes |
...
- `KAAPPI_WS` — multi-repo workspace root; supplies uninstalled
  ...
  release tarballs). Anything unavailable is skipped with a notice, never
  failed.
...
or starts and stops a private one.
